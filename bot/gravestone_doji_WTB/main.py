"""
Bot 4 Main Execution Script
This script initializes and runs the Bot 4 trading strategy
...
from bot.bot_4.config_reader import ConfigReader
from bot.bot_4.strategy import Bot4Strategy
...
def execute_strategy_for_account(account: Dict[str, Any], config: Dict[str, Any], results: List[Dict[str, Any]]):
    ...
    try:
        ...
        strategy = Bot4Strategy(groww, utils, account_config)
...
def main():
    """
    Main execution function for Bot 4
    """
    try:
        print("=" * 80)
        print("BOT 4 - TRADING STRATEGY")
        print("=" * 80)
        ...
        print(f"OTM Points: {config.get('otm_points')}")
        ...
        print(f"\n{'=' * 80}")
        print("BOT 4 EXECUTION COMPLETED")
        print(f"{'=' * 80}\n")

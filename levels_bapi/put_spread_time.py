...
from constant import put_spread_level_1, put_quantity_in_lots, put_spread_level_2, put_sl_level, put_target_level
...
class PutSpreadTrade:
    def __init__(self, groww: GrowwAPI):
        ...
        self.close_put_spreads_trade = False
        self.put_spread_strike_price_1 = None
        self.put_spread_quantity_1 = None
        self.put_spread_strike_price_2 = None
        self.put_spread_quantity_2 = None

    def put_spread(self):
        print("PUT Spread")
        self.open_first_put_spread_trade()
        self.open_second_put_spread_trade()
    ...
    def open_first_put_spread_trade(self):
        while not self.opened_first_trade:
            ...
            if spot_price <= put_spread_level_1:
                print(f"Sell trigger hit! Placing spread at {spot_price}")
                quantity = put_quantity_in_lots
                if put_spread_level_2:
                    quantity = quantity / 2
                ...
                if quantity == 0:
                    ...
                    exit(1)
                self.utils.place_put_spread(atm_strike, quantity * 65)
                self.opened_first_trade = True
                self.put_spread_strike_price_1 = atm_strike
                self.put_spread_quantity_1 = quantity

            # Sleep for 1 second before next check
            time.sleep(1)

    def open_second_put_spread_trade(self):
        if put_spread_level_2:
            while not self.opened_second_trade:
                ...
                if spot_price <= put_spread_level_2:
                    print(f"Sell trigger hit! Placing spread at {spot_price}")
                    quantity = put_quantity_in_lots / 2
                    atm_strike = self.utils.get_atm_strike(spot_price)
                    print("Atm strike:", atm_strike)
                    self.utils.place_put_spread(atm_strike, quantity * 65)
                    self.opened_second_trade = True
                    self.put_spread_strike_price_2 = atm_strike
                    self.put_spread_quantity_2 = quantity
    ...
    def close_all_put_spreads_trades(self):
        ...
        if self.opened_first_trade or self.opened_second_trade:
            while not self.close_put_spreads_trade:
                ...
                print(f"15-min Candle - Open: {candle_open}, Low: {candle_close}, SL Level: {put_sl_level}")

                # Check if candle open or low breaches the stop loss level
                if (candle_open and candle_open <= put_sl_level) or (candle_close and candle_close <= put_sl_level):
                    ...
                    if self.opened_first_trade:
                        print(f"Closing first put spread trade at strike {self.put_spread_strike_price_1}")
                        self.utils.close_put_spread(self.put_spread_strike_price_1, self.put_spread_quantity_1 * 65)

                    # Close second trade if opened
                    if self.opened_second_trade:
                        print(f"Closing second put spread trade at strike {self.put_spread_strike_price_2}")
                        self.utils.close_put_spread(self.put_spread_strike_price_2, self.put_spread_quantity_2 * 65)

                    self.close_put_spreads_trade = True

from growwapi import GrowwAPI

from levels_bapi.utils import Utils
from spot_trade.call_trade import CallTrade
from spot_trade.constant import call_trade, put_trade
from spot_trade.put_trade import PutTrade

if __name__ == '__main__':
    access_token = "eyJraWQiOiJaTUtjVXciLCJhbGciOiJFUzI1NiJ9..AraQx96yAf9Qg1Ic6ubrrj3TxdBOFH5ciqC_Wm0bHjABlzDrWST_AOdnudIkG6GrCzkgirMBjWB_jPerytJsAQ"
    groww = GrowwAPI(access_token)

    if call_trade:
        CallTrade(groww).call_spread()
    if put_trade:
        PutTrade(groww).put_spread()

    utils = Utils(groww)
    utils.close_all_fno_trades()